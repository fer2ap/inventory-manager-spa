<script>
  ...
  let { initial = null, isDefault = false, onSubmit, onCancel, submitLabel = "Salvar" } = $props();

  let name = $state(initial?.name ?? "");
  let description = $state(initial?.description ?? "");
  let maxWeightEnabled = $state(isDefault ? false : (initial?.maxWeight != null));
  let maxWeight = $state(initial?.maxWeight ?? 20);
  ...
  async function handleSubmit(e) {
    ...
    error = null;
    if (!name.trim()) {
      error = "Informe o nome do inventário.";
      return;
    }
    submitting = true;
    try {
      await onSubmit({
        ...
        maxWeight: isDefault ? null : (maxWeightEnabled ? Number(maxWeight) : null),
      });
    } catch (err) {
...
<form onsubmit={handleSubmit} class="form">
  ...
  {#if !isDefault}
    ...
    {#if maxWeightEnabled}
      ...
      />
    {:else}
      <p class="hint">Sem limite definido — sem alertas de capacidade.</p>
    {/if}
  {:else}
    <p class="hint">O inventário padrão não tem limite de capacidade.</p>
  {/if}
